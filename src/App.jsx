...
export default function MissionEmailManager() {
  // ---------- State
  const [members, setMembers] = useState([
    { id: "s1", name: "Jane Student", role: "student", email: "jane.s@school.org", guardianEmail: "parent.jane@example.com" },
    { id: "s2", name: "John Student", role: "student", email: "john.s@school.org", guardianEmail: "parent.john@example.com" },
    { id: "c1", name: "Mr. Smith", role: "chaperone", email: "smith@example.com" },
  ]);
  ...
  const [editId, setEditId] = useState(null); // when not null, we're editing
  ...
  const filtered = useMemo(() => {
    ...
    return members
      .filter((m) => (roleFilter === "all" ? true : roleFilter === "students" ? m.role === "student" : m.role === "chaperone"))
  ...
  const onSubmit = (e) => {
    ...
    if (editId) {
      setMembers((prev) => prev.map((m) => (m.id === editId ? { ...m, name, role: formRole, email, guardianEmail: formRole === "student" ? guardianEmail : undefined } : m)));
    } else {
      const newMember = {
        id: generateId(),
        name,
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 grid gap-10 divide-y-[6px] divide-gray-700">
        ...
        <section className="card soft-neutral">
          ...
          <form className="grid md:grid-cols-6 gap-4 items-start" onSubmit={onSubmit}>
            ...
            <div className="flex gap-2 md:col-span-6 mt-3 justify-center">
              ...
              {editId ? (
                <button type="button" className="btn-border" onClick={resetForm}>Cancel edit</button>
              ) : (
                <button type="button" className="btn-border" onClick={resetForm}>Cancel</button>
              )}
            </div>
          </form>
        </section>
        ...
        <section className="card">
          ...
          <div className="grid md:grid-cols-3 gap-3 items-end">
            ...
            <div>
              <label className="lbl">Role filter</label>
              <select className="inp bg-white w-full" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}>
              ...
              <thead>
                <tr className="bg-gray-100 text-left">
                  <th className="th w-10">
                    <input type="checkbox" aria-label="Select visible" onChange={(e) => e.target.checked ? selectAllVisible() : clearSelection()} />
                  </th>
              ...
              <tbody>
                {filtered.map((m) => (
                  <tr key={m.id} className="border-b last:border-0 hover:bg-gray-50">
                    <td className="td">
                      <input type="checkbox" checked={selected.has(m.id)} onChange={() => toggleOne(m.id)} aria-label={`Select ${m.name}`} />
                    </td>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td className="td text-center italic text-gray-500" colSpan={6}>No people found.</td>
                  </tr>
                )}
              </tbody>
            </table>
